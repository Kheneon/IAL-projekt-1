...
void List_Init( List *list ) {
	//Pouze nastavi prvky v listu na NULL
...
void List_Dispose( List *list ) {
	while(TRUE){
		...
		//Ulozeni ukazatel na prvek za prvnim prvkem, uvolneni prvniho prvku
		//a nasledne nahrani ukazatele za prvnim prvkem na misto prvniho prvku
		ListElementPtr list_elem;
		list_elem = list->firstElement->nextElement;
		free(list->firstElement);
		list->firstElement = list_elem;
	}
}
...
void List_InsertFirst( List *list, int data ) {
	ListElementPtr list_elem;
	list_elem = malloc(sizeof(struct ListElement));
	if(list_elem == NULL){
		List_Error();
	}
	else
	{
		list_elem->data = data;
		list_elem->nextElement = list->firstElement;
		list->firstElement = list_elem;
	}
}
...
void List_DeleteFirst( List *list ) {
	// Kontrola neprazdneho listu
	if(list->firstElement != NULL){
		//Kontrola, jestli se nerusi aktivita listu
		if(list->activeElement == list->firstElement){
			list->activeElement = NULL;
		}
		
		//Ulozeni ukazatele na prvni prvek v seznamu
		ListElementPtr list_elem;
		list_elem = list->firstElement;
...
void List_DeleteAfter( List *list ) {
	// Kontrola aktivity seznamu a jestli za aktivnim elementem se nachazi dalsi element
	if(list->activeElement != NULL){
		if(list->activeElement->nextElement != NULL){
			//Ulozeni elementu ktery se nachazi za next elementem aktivniho elementu
			//Ciselne se da vyjadrit jako active element + 2
			ListElementPtr list_elem;
			list_elem = list->activeElement->nextElement;
			list_elem = list_elem->nextElement;
			free(list->activeElement->nextElement);
			list->activeElement->nextElement = list_elem;
		}
	}
}
...
void List_InsertAfter( List *list, int data ) {
	if(list->activeElement != NULL){
		...
		{
			// vlozime element mezi aktivni a ten, co byl hned po aktivnim prvku
			list_elem->data = data;
			list_elem->nextElement = list->activeElement->nextElement;
			list->activeElement->nextElement = list_elem;
		}
	}
}
